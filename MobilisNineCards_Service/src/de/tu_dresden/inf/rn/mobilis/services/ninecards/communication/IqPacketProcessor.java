...
public class IqPacketProcessor
{
	...
	private void onGetGameConfigration(GetGameConfigurationRequest inBean) {
		if (mServiceInstance.getGame().getGameState() == Game.State.READY) {
			...
			XMPPBean out = new GetGameConfigurationResponse();
            out.errorType = XMPPBean.ERROR_TYPE_CANCEL;
            out.errorCondition = XMPPBean.ERROR_CONDITION_BAD_REQUEST;
            out.errorText = "Operation not allowed in this state.";
	...
	{
		if (mServiceInstance.getGame().getGameState() == Game.State.UNINITIALIZED) {
			
			mServiceInstance.getSettings().setRounds(inBean.getRounds());
			mServiceInstance.getSettings().setMaxPlayers(inBean.getPlayers());

			mServiceInstance.getGame().setGameState(Game.State.READY);
			...
			XMPPBean out = new ConfigureGameResponse();
            out.errorType = XMPPBean.ERROR_TYPE_CANCEL;
            out.errorCondition = XMPPBean.ERROR_CONDITION_BAD_REQUEST;
            out.errorText = "Operation not allowed in this state.";
